...
const AllCategory = (props) => {
  ...
  const [tabId, setTabId] = useState(""); // to highlight selected category
    ...
    return products.map((item, i) => {
      return (
        <Grid key={i} item xs={6} md={2.8} style={{ height: matches ? 240 : 300 }}>
          <Card onClick={()=>handleExploreProduct(item)} sx={{ border: "1px solid #ced4d6", height: "95%", width: "90%", boxShadow: "none",cursor:'pointer' }}>
