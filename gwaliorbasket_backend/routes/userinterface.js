...
router.post("/fetch_products", function (req, res) {
  console.log("category",req.body.categoryid)
    pool.query("select PL.*, (select P.pricetype from products P where P.productid=PL.productid) as pricetype, (select P.productname from products P where P.productid=PL.productid) as productname,(select P.image from products P where P.productid=PL.productid) as productimage from listproduct PL where PL.categoryid=?", [req.body.categoryid], function (error, result) {
      if (error){
        console.log(error)
  ...
  router.post("/fetch_all_productlist_by_product", function (req, res) {
      ...
      pool.query("select PL.*, (select P.pricetype from products P where P.productid=PL.productid) as pricetype, (select P.productname from products P where P.productid=PL.productid) as productname,(select P.image from products P where P.productid=PL.productid) as productimage from listproduct PL where PL.productid=?", [req.body.productid], function (error, result) {
        if (error){
          console.log(error)
           res.status(500).json({ status: false, message: "Server Error..." })
          }
    ...
    router.post("/fetch_products_by_category", function (req, res) {
      console.log("category",req.body.categoryname)
        pool.query("select PL.*, (select P.pricetype from products P where P.productid=PL.productid) as pricetype, (select P.productname from products P where P.productid=PL.productid) as productname,(select P.image from products P where P.productid=PL.productid) as productimage from listproduct PL where PL.categoryid=(select categoryid from categories where categoryname=? )", [req.body.categoryname], function (error, result) {
          if (error){
            console.log(error)
