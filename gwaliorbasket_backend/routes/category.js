...
router.post('/add_company_category',upload.single('icon'), function(req, res, next) {
  ...
  pool.query("insert into categories(companyid, categoryname, description, icon, createdat, updatedat, createdby)values(?,?,?,?,?,?,?)",[req.body.companyid,req.body.categoryname,req.body.description,req.file.originalname,req.body.createdat,req.body.updateat,req.body.createdby],function(error,result){
   if(error)
